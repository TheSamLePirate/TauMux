...
| `useSessionModel` | `PI_HT_BRIDGE_USE_SESSION_MODEL` | `true`         |
...
By default the active label and agent-end summaries are generated by the same
model the pi session is using — switching pi's model also switches the
summariser, with no config edit. Set `useSessionModel: false` (or
`PI_HT_BRIDGE_USE_SESSION_MODEL=0`) to pin a fast model via `provider` /
`modelId` instead. The `provider` / `modelId` pair is also used as a fallback
when the session has no model resolved yet.
