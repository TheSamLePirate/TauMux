...
| `HT_SOCKET_PATH` | `<config dir>/hyperterm.sock` | Override the Unix socket path. Must match between τ-mux and `ht` CLI. Since 0.3.187 the default is derived from the config dir (macOS: `~/Library/Application Support/hyperterm-canvas/hyperterm.sock`), so `ht` connects from any shell — not just panes the app spawned. |
