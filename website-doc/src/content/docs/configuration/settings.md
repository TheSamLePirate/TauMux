...
| `rpcSocketRequireToken` | boolean | `true` | **ON by default since v0.4.12.** The `ht` Unix socket requires a per-boot token for state-mutating commands (typing into panes, killing processes, installing extensions); read-only diagnostics stay open so `ht doctor` still works. The bundled `ht` + pi/Claude bridges + extension SDK present it automatically, so no first-party workflow changes; turn it off only for a third-party client that speaks the socket protocol directly and has not been taught to send `__token`. Defense-in-depth against opportunistic same-user processes, not a hard boundary. Settings → Network → "Require RPC socket token". *(added v0.3.163; default flipped v0.4.12)* |
| `terminalRenderer` | `"dom"` \| `"webgl"` | `"dom"` | **Experimental.** Opt-in GPU renderer for terminal panes (xterm.js WebGL addon) with automatic DOM fallback on unsupported hardware, init failure, or context loss — the settings panel shows a live "running on DOM — reason" hint when fallen back. It shipped enabled in v0.4.9 and rendered panes blank on some setups; a one-time migration in v0.4.12 reset persisted `webgl` back to `dom` (re-enabling afterwards sticks). Command palette: "Use WebGL/DOM Terminal Renderer". *(added v0.4.9)* |
