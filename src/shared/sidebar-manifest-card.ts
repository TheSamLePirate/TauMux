// Shared renderer for the sidebar workspace "manifest" card. Drives
// both the package.json (npm) path and the Cargo.toml (rust) path.
//
// Originally lived under `src/views/terminal/sidebar-manifest-card.ts`
// where it bound directly to the native `createIcon` factory. M14 of
// the web-mirror parity plan moved it here so the web client can
// reuse the exact same DOM shape (and downstream `.workspace-package*`
// CSS) without bringing in the full native icon set. The icon factory
// is now passed as a dependency; the native side hands over its
// existing `createIcon` as a one-line bind, the web client supplies a
// 4-glyph shim.
//
// Action click handler still dispatches the `ht-run-script` window
// CustomEvent — listeners on either surface decide what to do. M14
// of the parity plan wires a deferred-toast listener in the web
// mirror; native already has its run-script handler in
...
/** Just the icon names this module asks for. Surface's `createIcon`
 *  must accept these identifiers; native is a strict superset, web
 *  ships a 4-glyph shim. */
export type ManifestIconName =
  | "package"
  | "rocket"
  | "chevronDown"
  | "chevronRight";

export interface ManifestCardDeps {
  createIcon(name: ManifestIconName, cls: string, size: number): Element;
}

export function renderManifestCard(
  props: ManifestCardProps,
  deps: ManifestCardDeps,
): HTMLElement {
  const card = document.createElement("div");
  card.className = `workspace-package workspace-manifest-${props.kind}${props.expanded ? " expanded" : ""}`;

  card.appendChild(buildHeader(props, deps));
  ...
  if (props.actions.length > 0) {
    body.appendChild(buildActions(props, deps));
  }
...
function buildHeader(
  ...
  icon.append(
    deps.createIcon(props.kind === "cargo" ? "rocket" : "package", "", 11),
  );
  ...
  caret.append(
    deps.createIcon(props.expanded ? "chevronDown" : "chevronRight", "", 10),
  );
...
function buildActions(
  ...
  for (const action of props.actions) {
    ...
    if (action.state === "running") {
      ...
      run.textContent = "run";
      run.append(deps.createIcon("chevronRight", "", 9));
    }
    row.appendChild(run);

    row.addEventListener("click", (e) => {
      e.stopPropagation();
      // The host listens to `ht-run-script` (existing contract); native
      // spawns a fresh surface, web fires a deferred-toast (M14).
      // `scriptKey` is namespaced so npm "build" and cargo "build"
      // coexist in scriptErrors without colliding.
