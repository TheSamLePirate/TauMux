...
// M15 — `createIcon` was a top-level import that bound this module to
// the native icon set. It's now an injected dep so the web mirror can
// pass its own 1-glyph inline-SVG shim. When omitted, the close
...
class OverlayStack {
  ...
  constructor(
    ...
    private createIcon: CreateIconFn | undefined,
  ) {
  ...
  private relayout(): void {
    ...
    for (const payload of visible) {
      ...
      if (!ctrl) {
        ctrl = new OverlayCardController(
          payload,
          this.options,
          this.hooks,
          this.createIcon,
...
class OverlayCardController {
  ...
  private build(): HTMLElement {
    ...
    if (this.createIcon) {
      closeBtn.append(this.createIcon("close", "", 12));
    } else {
...
export class NotificationOverlay {
  ...
  /** M15 — injected icon factory. The native shim binds its existing
   *  `createIcon`; the web bridge passes a 1-glyph inline-SVG shim.
   *  Optional so unit tests + the test harness don't have to thread it
   *  through. */
  protected createIcon: CreateIconFn | undefined;

  constructor(
    private hooks: OverlayHooks,
    deps?: { createIcon?: CreateIconFn },
  ) {
    this.createIcon = deps?.createIcon;
  }
