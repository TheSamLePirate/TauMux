...
import { parseStatusKey } from "./status-key";
import { Meter } from "./tau-meter";
import { renderStatusEntry } from "./status-render";
...
/** Just the fields the registry actually reads. Narrowing avoids
 *  forcing the web mirror to materialize a full `AppSettings`
 *  (sensitive fields like `webMirrorAuthToken` are intentionally never
 *  broadcast). `statusBarKeys` is exposed because tooling
 *  (`tauDumpStatus` debug helper, future settings introspection)
 *  benefits from picking it off the shared context. */
export interface StatusContextSettings {
  htStatusKeyOrder?: readonly string[];
  htStatusKeyHidden?: readonly string[];
  statusBarKeys?: readonly string[];
}

export interface StatusContext {
  settings: StatusContextSettings;
...
// M12 — `model` and `kind` keys query DOM nodes the web mirror's
// container layout doesn't expose (`.surface-container[data-surface-id]`,
// `.tau-pane-agent`). They live on the native side via
// `src/views/terminal/native-status-keys.ts` which calls
// `registerStatusKey` at startup, keeping the shared registry pure-data.
...
// ── registry ─────────────────────────────────────────────────
//
// M12 — `model` + `kind` are NOT in the shared base because they query
// DOM nodes the web mirror's pane chrome doesn't expose. The native
// side calls `registerStatusKey` from `src/views/terminal/native-status-keys.ts`
// at startup to add them; the web mirror simply never sees those ids.
...
/** Inject a status-key renderer into the registry. The native module
 *  uses this at boot to add the DOM-coupled `model` + `kind` keys.
 *  Idempotent: calling it twice with the same id replaces the prior
 *  entry. Returns true if a previous registration was overwritten. */
export function registerStatusKey(renderer: StatusKeyRenderer): boolean {
  const existed = renderer.id in REGISTRY;
  REGISTRY[renderer.id] = renderer;
  return existed;
}
...
/** Snapshot of the registry's metadata, ordered by insertion. Computed
 *  on every read so the settings-panel picker reflects later
 *  `registerStatusKey` injections. Cheap (≤30 entries). */
export function getStatusKeyMeta(): StatusKeyMeta[] {
  return Object.values(REGISTRY).map(({ id, label, description, group }) => ({
    id,
    label,
    description,
    group,
...
/** Eager export for backwards compatibility. Note: this snapshots the
 *  registry at import time. Callers that need to see late-registered
 *  keys (the settings panel after `registerStatusKey('model', …)`) must
 *  use `getStatusKeyMeta()` instead. */
export const STATUS_KEY_META: StatusKeyMeta[] = getStatusKeyMeta();
