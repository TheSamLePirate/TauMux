...
| 11 | E.2 — M11 document pi-agent stream divergence | landed | (pending) | Docs-only — no version bump. New § 9 in `doc/system-pty-session.md` and a one-liner in CLAUDE.md explain why pi-agent-manager uses stdin/stdout rather than the fd 3/4/5 sideband. |
