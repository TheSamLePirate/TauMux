...
- **D.3** — service-worker update flow with toast. Skipped this pass: the plan rates this **L effort, medium risk** and explicitly calls for "a careful manual test pass on at least Chrome and Safari" — which can't happen in this automated run. The plan's recommendation also references "the mirror's existing toast machinery" that does not currently exist (`grep -rn 'toast' src/web-client/` returns nothing), so option 2 would need to land that infrastructure first. Recommend tackling D.3 in a focused session with a real browser open: build the toast UI module, then wire the SW `tau-mux-update-available` `CustomEvent`, then test cache invalidation across at least one Chrome and one Safari refresh cycle.
