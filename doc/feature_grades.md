...
Phase 7 polish — session 19 landed: Cluster H double-chunk on the notification + surface chip surfaces — notification overlay + sidebar item migrated to a new `--ht-notif-*` token group (8 tokens, including two `-bg-mix` tokens designed to live inside CSS color-mix()); surface chips migrated to a `--ht-chip-*` neutral pair + 4 extensions to the S18 `--ht-badge-*` family, REUSING the cross-component badge tokens exactly as designed. audit:theming 875 → 849 (-26 this session). Cumulative P7: 52 lifts across 19 sessions. **Cross-component design wins**: PM badges (S18) and surface chips (S19) both share the same `--ht-badge-success/warn/info-*` tokens — a single palette swap repaints the success/warn/info family across both. Remaining P7 long tail: Cluster H literal migration (~849 left, multi-session) + theme-preset interlock factory (F.6 final). Owned across future sessions of P7.
...
- **Gaps to AAA:**
  ...
  - Long-tail migration of ~1013 hard-coded colour literals in component CSS to tokens. P7 S9 started with notify-glow + notification-overlay; S10 added the sidebar workspace-card region (`--ht-sidebar-*`); S11 added the surface pane chrome region (`--ht-surface-*`); S12 added the small icon button region (`--ht-button-*`); S13 added the agent panel region (`--ht-agent-*`); S14 added the telegram pane region (`--ht-telegram-*`); S15 added the titlebar toolbar + sidebar inset region (`--ht-titlebar-*`); S16 added the command palette + kbd cheat-sheet region (`--ht-palette-*`); S17 added the ask-user modal + workspace-ask badge region (`--ht-ask-*`); S18 added the process manager chrome + kill-button region (`--ht-pm-*`) plus a cross-component semantic-badge namespace (`--ht-badge-*`); S19 added the notification overlay + sidebar item region (`--ht-notif-*`) and the surface chip region — surface chips REUSE the `--ht-badge-*` namespace from S18 (first cross-component reuse landing). audit:theming count: 1013 → 849 (−164 across S9–S19). Multi-session for the rest.
...
- **Grade: A**
- **Evidence:** `settings-manager.ts` — 500 ms debounced save; atomic writes (mode 0o600 since H.1); corrupt-file `.bak` recovery; partial-schema merge. Phase 6 catalogue ties the L7 atomic-write behaviour to `tests/atomic-write.test.ts` + the S1 mode invariant to `tests/file-modes.test.ts`. Bloom migration via `applyBloomMigration`. P7 S13 introduced the typed `FieldSchema<T>` seam in `src/shared/settings.schema.ts` with `numberRange()` / `bool()` factories (10 primitive fields). P7 S14 extended with `boolStrict()` + `numberRangeStrict()` factories (15 fields). P7 S15 added `enumStr<T>()` + `stringTrim()` + `stringArray()` factories (18 more fields). P7 S16 added `string()` + `nullableString()` factories (6 more fields, closing silent gaps where terminalBloom / cursorBlink / autoStartWebMirror / shellPath / fontFamily previously slipped through validateSettings via the unmodified `...s` spread with NO guard at all). P7 S17 added the `wrapped()` factory and `AUTO_CONTINUE_SCHEMA` so the autoContinue nested record flows through the same uniform call site as every other field — completes F.6 at **50 / ~50 simple settings fields, 100% coverage of the non-interlock surface**. Only the theme-preset interlock (themePreset gates 6 colour fields together) remains on the per-clause path. Tests pin clamp / round / strict-bool / strict-number / enum / trim / array / pass-through / nullable / wrapped semantics.
- **Gaps to AAA:**
  - Schema-version field + ordered migrations (currently a single `applyBloomMigration` with no version metadata).
  - Theme-preset interlock factory — themePreset / accentColor / secondaryColor / foregroundColor / bgBase / ansiColors share a derived-from-preset relationship; needs a `derived(fields => ...)` factory shape that takes the partial settings record.
