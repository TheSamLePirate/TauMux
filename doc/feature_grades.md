...
Phase 7 polish — session 37 landed: Cluster H **double-chunk** — vNext UI overrides (palette/prompt/search + process-manager/surface-details). 5 new --ht-vnext-text-* tokens cover the cooler 232/238/248 + 243/246/253 zinc scale used by the post-Phase-6 redesign (mute 0.48 → muted 0.58 → bright 0.88 → section-h 0.46 → emph 0.94); 2pp internal harmonisations fold 0.56 + 0.9 into adjacent tiers. ~28 literals migrated. audit:theming 568 → 543 (-25 this session). Cumulative P7: 88 lifts across 37 sessions. Remaining P7 long tail: Cluster H literal migration (~543 left, multi-session) + Cluster F.10. Owned across future sessions of P7.
...
- **Gaps to AAA:**
  ...
  - Long-tail migration of ~1013 hard-coded colour literals in component CSS to tokens. P7 S9–S37 walked the long tail by region (most recent stops): sidebar v2 workspace-script-btn + footer + server-dot (S35), t3 window override pure-reuse + window-shell base colours (S36; strongest single-block reuse demo), vNext palette/prompt/search + PM/surface-details overrides (S37; 5 new --ht-vnext-text-* tokens introduce the cooler 232/238/248 zinc scale used by the post-Phase-6 redesign, complete mute→muted→bright→section-h→emph ladder with 2pp internal harmonisations). audit:theming count: 1013 → 543 (−470 across S9–S37). Multi-session for the rest.
