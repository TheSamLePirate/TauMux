...
Phase 7 polish — session 14 landed: F.6 batch 2 (15 more settings fields migrated onto the FieldSchema seam — 11 strict-bool via new `boolStrict()` factory, 3 strict-number via new `numberRangeStrict()` factory, plus the two telegram notification toggles; cumulative 25 / ~50 settings fields migrated); Cluster H telegram pane region migrated to a new `--ht-telegram-*` token group (9 tokens covering toolbar / chat-select / message bubbles / composer / send-button; audit:theming 963 → 949). Cumulative P7: 42 lifts across 14 sessions. **A6 migration complete** (51 → 0 native producers). **F.6 seam landed S13 + extended S14** — half the settings fields now declared via schema. Remaining P7 long tail: Cluster H literal migration (~949 left, multi-session) + remaining ~25 settings fields (enum + array + string clauses). Owned across future sessions of P7.
...
- **Gaps to AAA:**
  ...
  - Long-tail migration of ~1013 hard-coded colour literals in component CSS to tokens. P7 S9 started with notify-glow + notification-overlay; S10 added the sidebar workspace-card region (`--ht-sidebar-*`); S11 added the surface pane chrome region (`--ht-surface-*`); S12 added the small icon button region (`--ht-button-*`); S13 added the agent panel region (`--ht-agent-*`); S14 added the telegram pane region (`--ht-telegram-*`). audit:theming count: 1013 → 949 (−64 across S9–S14). Multi-session for the rest.
