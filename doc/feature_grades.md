...
Phase 7 polish — session 25 landed: **Cluster B U10 closed** — every settings field now ships with a quiet "↺" reset-to-default affordance in the label wrap (visible only when value differs from DEFAULT_SETTINGS; JSON-shape compare so nested-record/array fields work). Wired through every field helper (text/number/slider/toggle/select/segmented/color/secret). Cluster H surface bar (pane header) chrome migrated to 4 new `--ht-surface-bar-*` tokens + 3 cross-component reuses (`--ht-badge-info-border-soft` focused border, `--ht-agent-row-bg-hover` focused inset, `--ht-sem-error` close-hover red). audit:theming 792 → 785 (-7 this session). Cumulative P7: 64 lifts across 25 sessions. Remaining P7 long tail: Cluster H literal migration (~785 left, multi-session) + Cluster F.10 + a few Cluster B/E residuals. Owned across future sessions of P7.
...
- **Gaps to AAA:**
  ...
  - Long-tail migration of ~1013 hard-coded colour literals in component CSS to tokens. P7 S9–S25 walked the long tail by region: notify-glow + notification-overlay (S9), sidebar workspace-card (S10, `--ht-sidebar-*`), surface pane chrome (S11, `--ht-surface-*`), small icon button (S12, `--ht-button-*`), agent panel (S13, `--ht-agent-*`), telegram pane (S14, `--ht-telegram-*`), titlebar toolbar + sidebar inset (S15, `--ht-titlebar-*`), command palette + kbd cheat-sheet (S16, `--ht-palette-*`), ask-user modal + workspace-ask badge (S17, `--ht-ask-*`), process manager chrome + kill-button (S18, `--ht-pm-*`) plus cross-component semantic-badge namespace (`--ht-badge-*`), notification overlay + sidebar item (S19, `--ht-notif-*`) plus surface chips, workspace cwd chip (S20, `--ht-cwd-chip-*`), workspace package card + cargo icon (S21, `--ht-package-*` / `--ht-cargo-icon`), workspace script-button states (S22, `--ht-script-*`), titlebar gradient + sidebar header text ladder (S23, `--ht-titlebar-gradient-*` + `--ht-sidebar-text-*`), surface-details overlay (S24, biggest cross-component reuse — REUSES S18 PM + badge + sem directly; +4 neutral secondary-button tokens), surface bar chrome (S25, `--ht-surface-bar-*` + 3 cross-component reuses). audit:theming count: 1013 → 785 (−228 across S9–S25). Multi-session for the rest.
...
### Audits
- **Grade: S**
- **Evidence:** `audits.ts:22-46` — async check + optional fix pattern; 5 s timeouts with explicit `proc.kill` on expiry (G.10). P7 S4 added `runAndPublishAudits()` so flipping `auditsGitUserNameExpected` (or any future audit-relevant setting) re-runs + refreshes the health snapshot without a restart. P7 S24 tripled the canary set: alongside the original `git-user-name`, `defaultAudits()` now registers `locale-utf8` (LC_ALL/LANG must be UTF-8, else subprocess output renders as ???), `bun-on-path` (probes `bun --version`; script-runner panes shell out to it), and `shell-exists` (stats `$SHELL`, warns if missing). Each new audit accepts an injected probe / env / file-exists hook so tests stay hermetic. Tests in `tests/audits.test.ts` (+12) + `tests/audits-auto-rerun.test.ts` + `tests/audits-timeout.test.ts`.
- **Gaps to AAA:**
  - Remediation UX hookup — three new audits intentionally ship without `fix` because the remedies live outside the app's reach (shell rc files, brew install bun).
