...
Phase 7 polish — session 16 landed: F.6 batch 4 (final easy fields migrated — `string()` + `nullableString()` factories added; terminalBloom / cursorBlink / autoStartWebMirror / shellPath / fontFamily / auditsGitUserNameExpected folded onto the schema seam — closes silent gaps where 5 fields previously slipped through validateSettings via the unmodified `...s` spread with no guard at all; cumulative 49 / ~50 fields, 98% coverage); Cluster H command palette + kbd cheat-sheet migrated to a new `--ht-palette-*` token group (14 tokens shared between the two sister overlays; audit:theming 944 → 922, -22). Cumulative P7: 46 lifts across 16 sessions. **F.6 seam now covers 98% of settings fields** — only the theme-preset interlock + autoContinue nested validator remain. Remaining P7 long tail: Cluster H literal migration (~922 left, multi-session) + theme-preset interlock factory. Owned across future sessions of P7.
...
- **Gaps to AAA:**
  ...
  - Migrate autoContinue (currently delegated to `validateAutoContinue` helper) — needs a `validatorWrapper()` factory shape.
