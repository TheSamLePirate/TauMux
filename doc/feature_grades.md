...
Phase 7 polish — session 42 landed: Cluster H **double-chunk** — pi-agent welcome + messages (5 new --ht-agent-msg-* tokens) and pi-agent think/code/tool-call (10 new --ht-agent-* tokens). **Third single-session drop record in a row: -53 literals** (S40 -44, S41 -47, S42 -53). The --ht-agent-* family now totals 23 tokens covering the entire pi-agent panel from welcome through code/think/tc/footer. diff/icon/inline color triples unify on --ht-sem-success / --ht-sem-error. audit:theming 382 → 329 this session. Cumulative P7: 98 lifts across 42 sessions. Remaining P7 long tail: Cluster H literal migration (~329 left, multi-session) + Cluster F.10. Owned across future sessions of P7.
...
- **Gaps to AAA:**
  ...
  - Long-tail migration of ~1013 hard-coded colour literals in component CSS to tokens. P7 S9–S42 walked the long tail by region (most recent stops): vNext titlebar/sidebar header + workspace-item card (S40, -44), browser pane + pi-agent toolbar/badges/stats (S41, -47), pi-agent welcome + messages + think + code + tool-call panel (S42, -53 — third record in a row). The --ht-agent-* family now totals 23 tokens covering the entire pi-agent panel; diff/icon/inline colors unify on --ht-sem-success / --ht-sem-error. audit:theming count: 1013 → 329 (−684 across S9–S42). Theme-token test suite now at 574 tests. Multi-session for the rest.
