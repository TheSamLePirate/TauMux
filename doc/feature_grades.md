...
Phase 7 polish — session 13 landed: F.6 typed `FieldSchema` seam in `src/shared/settings.schema.ts` (10 primitive fields migrated as proof — fontSize / lineHeight / terminalBgOpacity / bloomIntensity / paneGap / sidebarWidth / webMirrorPort / scrollbackLines / notificationSoundEnabled / notificationSoundVolume — remaining ~40 fields stay on the per-clause path for incremental fold-in); Cluster H agent panel region migrated to a new `--ht-agent-*` token group (6 tokens; audit:theming 971 → 963). Cumulative P7: 40 lifts across 13 sessions. **A6 migration complete** (51 → 0 native producers, 100%). **F.6 seam landed** — long-standing deferred item closed. Remaining P7 long tail: Cluster H literal migration (~963 left, multi-session) + folding the remaining settings fields onto the schema seam. Owned across future sessions of P7.
...
- **Gaps to AAA:**
  ...
  - Long-tail migration of ~1013 hard-coded colour literals in component CSS to tokens. P7 S9 started with notify-glow + notification-overlay; S10 added the sidebar workspace-card region (`--ht-sidebar-*`); S11 added the surface pane chrome region (`--ht-surface-*`); S12 added the small icon button region (`--ht-button-*`); S13 added the agent panel region (`--ht-agent-*`). audit:theming count: 1013 → 963 (−50 across S9–S13). Multi-session for the rest.
...
- **Grade: A**
- **Evidence:** `settings-manager.ts` — 500 ms debounced save; atomic writes (mode 0o600 since H.1); corrupt-file `.bak` recovery; partial-schema merge. Phase 6 catalogue ties the L7 atomic-write behaviour to `tests/atomic-write.test.ts` + the S1 mode invariant to `tests/file-modes.test.ts`. Bloom migration via `applyBloomMigration`. P7 S13 introduced the typed `FieldSchema<T>` seam in `src/shared/settings.schema.ts`: 10 simple primitive fields (fontSize, lineHeight, terminalBgOpacity, bloomIntensity, paneGap, sidebarWidth, webMirrorPort, scrollbackLines, notificationSoundEnabled, notificationSoundVolume) now declare default + validator in a single place via `numberRange()` / `bool()` factories, and `validateSettings()` delegates through the schema for those fields. Tests pin the clamp / round / coerce semantics + assert schema defaults match `DEFAULT_SETTINGS`. Remaining ~40 fields stay on the per-clause path for incremental fold-in.
- **Gaps to AAA:**
  - Schema-version field + ordered migrations (currently a single `applyBloomMigration` with no version metadata).
  - Fold the remaining ~40 settings fields onto the new `FieldSchema` seam.
