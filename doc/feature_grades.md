...
Phase 7 polish — session 17 landed: F.6 batch 5 (autoContinue migrated through a new `wrapped()` factory — the seam now covers 50 / ~50 simple settings fields, only the theme-preset interlock remains); Cluster H ask-user modal + sidebar workspace-ask badge migrated to a new `--ht-ask-*` token group (14 tokens covering scrim / sheet / codebox / danger banner+button / cyan badge; audit:theming 922 → 907, -15). Cumulative P7: 48 lifts across 17 sessions. **F.6 seam now covers every simple field** — the only remaining settings holdout is the theme-preset interlock (themePreset gates 6 colour fields together). Remaining P7 long tail: Cluster H literal migration (~907 left, multi-session) + theme-preset derived() factory. Owned across future sessions of P7.
