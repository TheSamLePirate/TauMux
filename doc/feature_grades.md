...
Phase 7 polish — session 15 landed: F.6 batch 3 (18 more settings fields migrated onto the FieldSchema seam — 8 enums via new `enumStr<T>()` factory, 3 string-trim via new `stringTrim()` factory, 3 string-array via new `stringArray()` factory, 4 `!!`-bool fields; cumulative 43 / ~50 settings fields migrated — only ~7 special-case fields remain); Cluster H titlebar toolbar + sidebar inset migrated to a new `--ht-titlebar-*` token group (3 tokens) plus 2 `--ht-sidebar-*` tokens (audit:theming 949 → 944). Cumulative P7: 44 lifts across 15 sessions. **A6 migration complete** (51 → 0 native producers). **F.6 seam now covers 86% of settings fields.** Remaining P7 long tail: Cluster H literal migration (~944 left, multi-session) + remaining ~7 settings fields (auditsGitUserNameExpected, autoContinue, themePreset, ansiColors, terminalBloom, cursorBlink, shellPath etc — each has bespoke validation). Owned across future sessions of P7.
...
- **Grade: A**
- **Evidence:** `settings-manager.ts` — 500 ms debounced save; atomic writes (mode 0o600 since H.1); corrupt-file `.bak` recovery; partial-schema merge. Phase 6 catalogue ties the L7 atomic-write behaviour to `tests/atomic-write.test.ts` + the S1 mode invariant to `tests/file-modes.test.ts`. Bloom migration via `applyBloomMigration`. P7 S13 introduced the typed `FieldSchema<T>` seam in `src/shared/settings.schema.ts` with `numberRange()` / `bool()` factories (10 primitive fields). P7 S14 extended with `boolStrict()` + `numberRangeStrict()` factories (15 fields). P7 S15 added `enumStr<T>()` + `stringTrim()` + `stringArray()` factories — full coverage of the common validation patterns (18 more fields: 8 enums, 3 string-trim, 3 string-array, 4 `!!`-bool). Cumulative 43 / ~50 settings fields migrated; only ~7 special-case fields remain on the per-clause path. Tests pin clamp / round / strict-bool / strict-number / enum / trim / array semantics.
- **Gaps to AAA:**
  - Schema-version field + ordered migrations (currently a single `applyBloomMigration` with no version metadata).
  - Fold the remaining ~7 settings fields (auditsGitUserNameExpected — null|string|fallback; autoContinue — nested record with own validator; themePreset / accentColor / secondaryColor / foregroundColor / bgBase / ansiColors — theme-preset interlock; terminalBloom, cursorBlink, autoStartWebMirror, shellPath, fontFamily — straightforward, just hadn't been bundled in) onto the `FieldSchema` seam.
