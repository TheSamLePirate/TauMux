...
Phase 7 polish — session 18 landed: Cluster H double-chunk on the process manager (⌘⌥P) overlay — chrome migrated to a new `--ht-pm-*` token group (15 chrome + 6 kill-button tokens for the soft-red inline-action family, distinct from the modal-prompt --ht-ask-danger-* family); semantic badges (port / git-clean / git-dirty) pulled into a new cross-component `--ht-badge-*` namespace (7 tokens reusable by future surface chips + sidebar status pills). PM git-conflicts / git-add / CPU heatmap rewired onto existing `--ht-sem-error` / `--ht-sem-success` tokens — first migration where literals collapse onto already-defined sem tokens. audit:theming 907 → 875 (-32 this session). Cumulative P7: 50 lifts across 18 sessions. Remaining P7 long tail: Cluster H literal migration (~875 left, multi-session) + theme-preset interlock factory (F.6 final). Owned across future sessions of P7.
