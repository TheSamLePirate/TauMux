...
| 02 | [Smart status-key system (`_pct`, `_lineGraph`, `_array`, hidden `_`)](plan_status_keys_smart.md) | `src/views/terminal/status-keys.ts`, sidebar status renderer, `bin/ht set-status` | L      |
...
| 08 | [Telegram smart buttons on turn-end notifications](plan_telegram_smart_buttons.md) | `src/bun/telegram-service.ts`, `claude-integration/`, `pi-extensions/` | M      |
...
All plans are **proposed, not started**. No `tracking_*.md` files
exist yet; the first agent to start executing creates them.
Implementation gated on user prioritisation.
