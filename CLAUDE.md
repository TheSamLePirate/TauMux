...
- `pi-extensions/` — Pi coding-agent extensions. `ht-bridge/` (renamed from `ht-notify-summary/` in 0.2.80) surfaces pi turns into the sidebar via `ht set-status` + `ht notify`, and the larger integration plan in this directory adds tool-execution badges, plan mirroring, ask-user modals, custom LLM-callable tools, and a system-prompt primer.
