...
| L10 | `PiAgentInstance.kill()` is idempotent + crash-safe before `start()` (H12 removed the Promise-based `send()`/`responseWaiters` machinery it used to drain). | `src/bun/pi-agent-manager.ts` `kill()` | `tests/pi-agent-manager.test.ts` "kill() before start() marks the instance dead without throwing" |
